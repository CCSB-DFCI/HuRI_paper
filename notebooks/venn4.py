...
def venn(data, names=None, fill="number", show_names=True, show_plot=True, **kwds):
	...
	if len(data) == 2:
		...
		venn4(data, names=names, fill=fill, show_names=show_names, show_plot=show_plot, **kwds)
	else:
...
#--------------------------------------------------------------------
def venn4(data=None, names=None, TitleName = None, FileName = None, fill="number", show_names=True, show_plot=True, **kwds):
	...
	if 'colors' in kwds and isinstance(kwds['colors'], Iterable) and len(kwds['colors']) >= 4:
		...
		colors = ['forestgreen', 'darkviolet', 'firebrick', 'orange']
	...
	patches.append(Ellipse((200, 200), width, height, -45, color=colors[2], alpha=0.5))
	patches.append(Ellipse((200, 200), width, height, -135, color=colors[3], alpha=0.5))
	patches.append(Ellipse((230, 170), width, height, -135, color=colors[1], alpha=0.5))
	for e in patches:
		ax.add_patch(e)
	...
	leg = ax.legend([names[0],names[2],names[3],names[1]], loc='upper center', fancybox=True)
	leg.get_frame().set_alpha(0.5)
